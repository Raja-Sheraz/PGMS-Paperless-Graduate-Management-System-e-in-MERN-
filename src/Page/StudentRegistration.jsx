...
const StudentRegistration = () => {
  ...
  const validateForm = () => {
    ...
    // Validation for CNIC (exactly 13 dis)
    const cnicRegex = /^\d{13}$/;
    if (!cnicRegex.test(cnic)) {
      newErrors.cnic = "Please enter a valid CNIC with exactly 13 dis.";
      isValid = false;
    } else {
      newErrors.cnic = "";
    }

    // Validation for phone number (exactly 11 dis)
    const phoneRegex = /^\d{11}$/;
    if (!phoneRegex.test(phone)) {
      newErrors.phone =
        "Please enter a valid phone number with exactly 11 dis.";
      isValid = false;
    } else {
      newErrors.phone = "";
    }
